...
void sssp_init_work(SimpleCSRGraphUII g, unsigned int src, int start, int end){
  for(int i = start; i < end; i++) {
    g.node_wt[i] = (i == src) ? 0 : INF;
  }
}

void sssp_init(SimpleCSRGraphUII g, unsigned int src, int numth) {
...
void sssp_round_work(SimpleCSRGraphUII g, int start, int end, bool &changed) {
...
bool sssp_round(SimpleCSRGraphUII g, int numth){
  bool changed = false;
  std::vector<std::thread> ths;
  int work_per = ceil(g.num_nodes/(double)numth);
  for(int i = 0; i < numth; i++){
